...
func TestIsValidURLStandalone(t *testing.T) {
	tests := []struct {
		url      string
		expected bool
	}{
...
func TestShouldFilterItemStandalone(t *testing.T) {
	// テスト用のアイテム
	item1 := Item{
		Title: "正常な記事タイトル",
...
func TestFilterRSSStandalone(t *testing.T) {
	// テスト用のRSSデータ
	originalRSS := &RSS{
		Version: "2.0",
		Channel: Channel{
...
func TestParseRSSStandalone(t *testing.T) {
	// テスト用のRSS XML
...
func TestGenerateRSSStandalone(t *testing.T) {
	// テスト用のRSSデータ
	rss := &RSS{
		Version: "2.0",
		Channel: Channel{
